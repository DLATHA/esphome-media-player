...
Control multi-room Sonos speaker groups directly from the touchscreen panel. When the selected media player is a Sonos speaker, a speaker icon appears on the main screen. Tap it to open a panel where you can add or remove speakers from the group.
...
Two template sensor helpers need to be created in Home Assistant. This is done entirely through the UI — no YAML editing required.
...
- The speaker group button only appears when the selected media player is a Sonos device and there are at least two Sonos speakers
- If you change the selected media player to a non-Sonos device, the button hides automatically
- Swipe gestures (track skip, volume panel) are disabled while the grouping panel is open
